...
def analyser_article_avec_IA(article_info, prompt_analystgpt):
    ...
    prompt_complet = prompt_analystgpt.replace("Analysez cet article.", article_json)
    ...
    try:
        ...
        return response.choices[0].message.content if response.choices else "Aucune réponse valide reçue de AnalystGPT."
    except Exception as e:
...
for index, (article_key, article) in enumerate(articles.items()):
    ...
    if 'erreur' not in scraped_article:
        ...
        enregistrer_resultat(analysis_result, index + 1)
        print(f"Analyse de l'article {index + 1} terminée et enregistrée.")
    else:
